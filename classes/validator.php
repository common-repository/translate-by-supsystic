...
class validatorTbs {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldTbs') {
            $value = $field;
            $field = new fieldTbsTbs('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorTbs', $v)) 
                    self::$v($field);
            }
        }
        if(method_exists('validatorTbs', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorTbs');
        foreach($all as $m) {
