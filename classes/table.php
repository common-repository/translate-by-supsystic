...
abstract class tableTbs {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(TBS_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjTbs('fieldTbs', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbTbs::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbTbs::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbTbs::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothing to update', TBS_LANG_CODE));
            return false;
        }
        ...
        if(dbTbs::query($query)) {
            if($method == 'INSERT')
                return dbTbs::lastID();
            else
                return true;
        } else
			$this->_addError(TBS_TEST_MODE ? dbTbs::getError() : __('Database error. Please contact your developer.', TBS_LANG_CODE));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if(isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterTbs::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterTbs::DB);
                    if($validate) {
                        if ($this->_fields[$k] && is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorTbs::_($objForValidation)) {
    ...
    /**
     * Add new fieldTbsTbs for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjTbs('fieldTbs', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbTbs::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(TBS_DATE_FORMAT, dbTbs::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbTbs::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
