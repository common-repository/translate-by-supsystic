...
class utilsTbs {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsTbs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsTbs::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameTbs::_()->getModule($m)) 
                return false;
            if(!is_dir(frameTbs::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameTbs::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqTbs::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqTbs::getVar('HTTP_REFERER', 'server');
	}
	...
	static public function encodeArrayTxt($arr, $forceOrd = false) {
		if(function_exists('base64_encode') && !$forceOrd) {
			return array_map('base64_encode', $arr);
		}
		foreach($arr as $k => $v) {
			$len = strlen($v);
			$res = array();
			...
			$arr[$k] = implode('|', $res). ':ORD_ENC';
		}
		return $arr;
	}
	static public function decodeArrayTxt($arr, $checkDbValue = false) {
		foreach($arr as $k => $v) {
			if(strpos($v, ':ORD_ENC') !== false) {
				$resStr = '';
				$strChars = explode('|', str_replace(':ORD_ENC', '', $v));
				foreach($strChars as $ord) {
					$resStr .= chr($ord);
				}
				$arr[$k] = $resStr;
			} else if(function_exists('base64_decode') && (!$checkDbValue || ($checkDbValue && base64_encode(base64_decode($v)) == $v))) {
