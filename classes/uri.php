...
class uriTbs {
    ...
    static public function fileToPageParam($file) {
        ...
        return substr($file, strpos($file, TBS_PLUG_NAME));
    }
    static public function _($params) {
		...
		if(is_string($params) 
			...
			|| strpos($params, TBS_PLUG_NAME) !== false)	// If relative links in WP is used (by other plugin for example)
		) {
			if(self::isHttps())
    ...
    static public function atach($params) {
        ...
        if(!empty($getData)) {
            if(is_array($params)) {
				$mergerParams = $params;
				if(isset($mergerParams['baseUrl'])) unset($mergerParams['baseUrl']);
                $params = array_merge($getData, $params, $mergerParams);
			} else
