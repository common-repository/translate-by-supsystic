<?php
abstract class modelTbs extends baseObjectTbs {
	...
	public function getModule() {
		return frameTbs::_()->getModule( $this->_code );
	}
	...
	protected function _buildQuery($table = null) {
		if(!$table)
			$table = frameTbs::_()->getTable( $this->_tbl );
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameTbs::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				return true;
			} else 
				$this->pushError (__('Database error detected', TBS_LANG_CODE));
		} else
			$this->pushError(__('Invalid ID', TBS_LANG_CODE));
		return false;
	}
	...
	public function delete($params = array()) {
		if(frameTbs::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else 
			$this->pushError (__('Database error detected', TBS_LANG_CODE));
		return false;
	...
	public function insert($data) {
		...
		$id = frameTbs::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameTbs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById($data, $id = 0) {
		...
		if($id) {
			...
			$this->pushError(__('Empty or invalid ID', TBS_LANG_CODE));
		return false;
	}
	public function update($data, $where) {
		$data = $this->_dataSave($data, true);
		if(frameTbs::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameTbs::_()->getTable( $this->_tbl )->getErrors());
		return false;
