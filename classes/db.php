...
class dbTbs {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see TBS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, TBS_DB_PREF, $wpdb->prefix. TBS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampTbs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, TBS_DATE_DL)) return dateToTimestampTbs($date);
        $arr = explode('-', $date);
        return dateToTimestampTbs($arr[2]. TBS_DATE_DL. $arr[1]. TBS_DATE_DL. $arr[0]);
    }
