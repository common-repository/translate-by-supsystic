...
abstract class controllerTbs {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameTbs::_()->getModule( $this->getCode() );
		$className = '';
		if(importTbs($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameTbs($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importTbs($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameTbs($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(TBS_ADMIN),
				'remove' => array(TBS_ADMIN),
				'restore' => TBS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - TBS_METHODS or TBS_USERLEVELS
	 */
	...
	public function getModule() {
		return frameTbs::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqTbs::getVar('page');
		$rowsLimit = (int) reqTbs::getVar('rows');
		$orderBy = reqTbs::getVar('sidx');
		$sortOrder = reqTbs::getVar('sord');

		// Our custom search
		$search = reqTbs::getVar('search');
		...
		// jqGrid search
		$isSearch = reqTbs::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqTbs::getVar('searchField'));
			$searchString = trim(reqTbs::getVar('searchString'));
		...
		$res = dispatcherTbs::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseTbs();
		if($this->getModel()->removeGroup(reqTbs::getVar('listIds', 'post'))) {
