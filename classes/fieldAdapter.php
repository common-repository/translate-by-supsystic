...
class fieldAdapterTbs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterTbs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsTbs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsTbs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsTbs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameTbs::_()->getModule('products')->getCategories();
        $brands = frameTbs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', TBS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', TBS_LANG_CODE). '</b><br />
                '. __('Destination', TBS_LANG_CODE). ':'. htmlTbs::checkboxlist('params[dest]', array('optionsTbs' => $listOfDest)). '<br />
                '. __('Country', TBS_LANG_CODE). ':'. htmlTbs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', TBS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', TBS_LANG_CODE) : htmlTbs::checkboxlist('params[categories][]', array('optionsTbs' => $cOptions))). '<br />
                    <b>'. __('Brands', TBS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', TBS_LANG_CODE) : htmlTbs::checkboxlist('params[brands][]', array('optionsTbs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', TBS_LANG_CODE).': '. htmlTbs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', TBS_LANG_CODE).': '. htmlTbs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', TBS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameTbs::_()->getModule('optionsTbs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlTbs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', TBS_LANG_CODE).':<br />';
                $output .= htmlTbs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', TBS_LANG_CODE).':<br />';
                $output .= htmlTbs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsTbs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsTbs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', TBS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsTbs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameTbs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameTbs::_()->getTable('states')
				->leftJoin( frameTbs::_()->getTable('countries'), 'country_id' )
