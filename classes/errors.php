...
class errorsTbs {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqTbs::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $tbsErrors = reqTbs::getVar('tbsErrors');
        if(!empty($tbsErrors)) {
            if(!is_array($tbsErrors)) {
                $tbsErrors = array( $tbsErrors );
            }
            $tbsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $tbsErrors)));
            if(!empty($tbsErrors)) {
                self::$current = $tbsErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsTbs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsTbs', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqTbs::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqTbs::clearVar('sesErrors', 'session');
    }
