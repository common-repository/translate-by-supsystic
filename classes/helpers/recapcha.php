...
class recapchaTbs {
	...
	public function __construct() {
		if(!function_exists('recaptcha_get_html')) {	// In case if this lib was already included by another plugin
			importTbs(TBS_HELPERS_DIR. 'recaptchalib.php');
		}
	}
	static public function getInstance() {
		static $instance = NULL;
		if(empty($instance)) {
			$instance = new recapchaTbs();
		}
		return $instance;
	}
	...
	public function check() {
		$resp = recaptcha_check_answer($this->_privateKey, 
					$_SERVER['REMOTE_ADDR'], 
					reqTbs::getVar('recaptcha_challenge_field'),
					reqTbs::getVar('recaptcha_response_field'));
