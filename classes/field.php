...
class fieldTbs {
	 ...
	 */
	public $adapt = array('htmlTbs' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterTbs::_($value, $this->adapt['dbFrom'], fieldAdapterTbs::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlTbs', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterTbs::_($this, $this->adapt['html'], fieldAdapterTbs::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameTbs::_()->getModule('optionsTbs')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', TBS_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsTbs'] = $options;
				}
			}
			$htmlContent = htmlTbs::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlTbs::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterTbs::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterTbs::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsTbs');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameTbs::_()->getModule('optionsTbs')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsTbs'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlTbs', $method)) {
			$output .= htmlTbs::$method($name, $params);
			$output .= htmlTbs::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', TBS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', TBS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', TBS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterTbs::_($id,'getExtraFieldOptions',fieldAdapterTbs::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', TBS_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', TBS_LANG_CODE).':<br />';
					$output .= htmlTbs::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', TBS_LANG_CODE).':<br />';
					$output .= htmlTbs::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterTbs::_($this, $this->adapt['htmlParams'], fieldAdapterTbs::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', TBS_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', TBS_LANG_CODE).':<br />';
				$output .= fieldAdapterTbs::_($params,'getFieldAttributes',  fieldAdapterTbs::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsTbs::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperTbs');
		  $module = $this->checkVarFromParam($param,'moduleTbs');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameTbs::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameTbs::_()->getModule($module)->isExternal())
		   $config_xml = frameTbs::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = TBS_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', TBS_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameTbs::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsTbs'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlTbs::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
