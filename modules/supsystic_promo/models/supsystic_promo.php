<?php
class supsystic_promoModelTbs extends modelTbs {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbTbs::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbTbs::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(TBS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(TBS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbTbs::get($query, 'one');
	}
	...
	public function getTourHst() {
		$hst = get_user_meta(get_current_user_id(), TBS_CODE . '-tour-hst', true);
		if(empty($hst))
			$hst = array();
		if(!isset($hst['passed']))
			$hst['passed'] = array();
	...
	public function setTourHst( $hst ) {
		update_user_meta(get_current_user_id(), TBS_CODE . '-tour-hst', $hst);
	}
	public function clearTourHst() {
		delete_user_meta(get_current_user_id(), TBS_CODE . '-tour-hst');
	}
