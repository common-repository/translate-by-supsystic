<?php
class optionsViewTbs extends viewTbs {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = TBS_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. TBS_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. TBS_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameTbs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameTbs::_()->addJSVar('adminOptionsTbs', 'tbsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameTbs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		$this->assign('mainLink', frameTbs::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameTbs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameTbs::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameTbs::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameTbs::_()->getModule('options')->getAll();
