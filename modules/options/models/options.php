<?php
class optionsModelTbs extends modelTbs {
	...
	public function save($optKey, $val, $ignoreDbUpdate = false) {
		...
		if(!isset($this->_values[ $optKey ]) || $this->_values[ $optKey ]['value'] != $val) {
			...
			//frameTbs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('option.'. $optKey);
		}
	}
    ...
    public function saveGroup($d = array()) {
		if(isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			dispatcherTbs::doAction('beforeSaveOpts', $d);
			...
			$this->pushError(__('Empty data to save option', TBS_LANG_CODE));
        return false;
    }
	private function _updateOptsInDb() {
		update_option(TBS_CODE. '_opts_data', $this->_values);
	}
	private function _loadOptValues() {
		if(!$this->_valuesLoaded) {
			$this->_values = get_option(TBS_CODE. '_opts_data');
