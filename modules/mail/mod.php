<?php
class mailTbs extends moduleTbs {
	private $_smtpMailer = null;
	private $_sendMailMailer = null;

	public function init() {
		parent::init();
		//dispatcherTbs::addFilter('optionsDefine', array($this, 'addOptions'));
	}
	public function send($to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array()) {
		$type = frameTbs::_()->getModule('options')->get('mail_send_engine');
		$res = false;
      $type = 'wp_mail';
		switch($type) {
	...
	public function sendWpMail($to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array()) {
		...
		if(!function_exists('wp_mail'))
			frameTbs::_()->loadPlugins();
		if(!frameTbs::_()->getModule('options')->get('disable_email_html_type')) {
		...
		switch($type) {
			case 'wp_mail': default:
				...
				if(empty($ts_mail_errors)) {
					$ts_mail_errors[] = __('Cannot send email - problem with send server', TBS_LANG_CODE);
				}
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', TBS_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', TBS_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', TBS_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', TBS_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
