 ...
 */
if(!function_exists('dateToTimestampTbs')) {
    function dateToTimestampTbs($date) {
        if(empty($a))
            return false;
        $a = explode(TBS_DATE_DL, $date);
...
if(!function_exists('importTbs')) {
    function importTbs($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassTbs')) {
    function importClassTbs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), TBS_CODE) !== false) {
					$classFile = preg_replace('/'. TBS_CODE. '/i', '', $classFile);
				}
                $path = TBS_CLASSES_DIR. $classFile. '.php';
			}
            return importTbs($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, TBS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], TBS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameTbs')) {
    function toeGetClassNameTbs($class) {
        $className = '';
		if(class_exists($class. strFirstUp(TBS_CODE))) 
			$className = $class. strFirstUp(TBS_CODE);
		else if(class_exists(TBS_CLASS_PREFIX. $class)) 
            $className = TBS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjTbs')) {
    function toeCreateObjTbs($class, $params) {
        $className = toeGetClassNameTbs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectTbs')) {
    function redirectTbs($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsTbs')) {
	function prepareParamsTbs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsTbs::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeTbs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeTbs')) {
	function prepareFieldCodeTbs($string) {   
