...
jQuery.fn.serializeAnythingTbs = function(addData, returnArray) {
...
function tbs_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableTbs(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.tbsExample, .tbsTblHeader').remove();
		var exampleRow = this.table.find('.tbsExample');
		for(var i in list) {
			...
			newRow.removeClass('tbsExample').show();
...
function setCookieTbs(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.tbsZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive Wordpress attach ID from image, using img classes
