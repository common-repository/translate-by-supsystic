	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('tbsWpTabs')) {
					$this.addClass('tbsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.tbsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.tbsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.tbsTabContent');
					allTabsContent.hide();
